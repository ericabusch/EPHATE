...
class EPHATE(BaseEstimator):
    ...
    def __init__(self,
        ...
        **kwargs):
            ...
            self.X = None
            self.X2 = None
            self.X2_metric=X2_metric
            self.optimal_t = None
            self.diff_op = None
            self.exogenous_op = None
            self.exogenous_sim_matrix=None
            self.normalize_exog_features=normalize_exog_features
            self.n_jobs = n_jobs
    ...
    def set_params(self, **params):
        ...
        landmarking is not suggested with EPHATE as it reduces the number of samples
        ...
        EPHATE-specific parameters------------------------------------------
        X2_metric : string, optional, default: "euclidean"
        chooes from ['euclidean','cosine','pearsonr','spearmanr']
        selects what similarity metric is used for pairwise comparisons in exogenous view
        ...
        if "X2_metric" in params and params['X2_metric']!=self.X2_metric:
            self.X2_metric=params['X2_metric']
            del params['X2_metric']
            reset_kernel=True
            reset_potential=True
            reset_embedding=True

        ###### ELB FINISH SOMETHING HERE ###################
    ...
    #@property
    def set_exogenous_op(self):
        """ exogenous_op :  array-like, shape=[n_samples, n_samples] or [n_landmark, n_landmark]
        The exogenous view (second view of ephate). 
        """
        if self.exogenous_op is None:
            P, M = self.generate_exogenous_view()
            self.exogenous_op = M
            self.exogenous_sim_matrix = P
        return
        # else:
    ...
    def set_diff_op(self):
        ...
        The dual-diffusion ephate operator. Combination of the phate diffusion operator and the exogenous factors.
        """

        _logger.info(
                "Combining PHATE operator and exogenous matrix operator")
        if self.diff_op is None:
            if self.phate_diffop is not None and self.exogenous_op is not None:
                self.diff_op = np.matmul(self.phate_diffop.T, self.exogenous_op)
            else:
                raise NotFittedError(" ")
        return


    def generate_exogenous_view(self):
        # check if X2 (the exogenous matrix) is square
        # if it is square, we still have to check if symmetric

        if (self.X2.shape[0] != self.X2.shape[1]) or (not utils.is_Symmetric(self.X2)):
            # turn it into pairwise similarity function
            _logger.info("Computing pairwise similarity of exogenous features with {} metric".format(self.X2_metric))
            P = self.compute_pairwise_similarity_mtx()
        else:
            _logger.info("Using supplied exogenous features affinity mat")
            P = self.X2
        M = utils.row_normalize(P)
        return P, M 

    def compute_pairwise_similarity_mtx(self):
        '''
        computes the pairwise similarity metric for a given matrix of samples in X2
        ''' 

        euclid_func = lambda u, v: 1 - 2 * np.sqrt(np.sum((u-v)**2)) / (np.sqrt(np.sum(u ** 2)) + np.sqrt(np.sum(v ** 2)))
        cos_func = lambda u, v: np.dot(u, v) / (np.linalg.norm(u) * np.linalg.norm(v))
        corr_func = lambda u, v : (np.cov(u, v, ddof=0) / (np.std(u)*np.std(v)))[0][1] 
        spearman_func = lambda u, v: spearmanr(u, v).correlation
        simfunc = None
        if self.X2_metric == 'cosine':
            simfunc = cos_func
        elif self.X2_metric == 'pearsonr':
            simfunc = corr_func
        elif self.X2_metric == 'spearmanr':
            simfunc = spearman_func
        elif self.X2_metric == 'euclidean':
            simfunc = euclid_func
        else:
            raise NotFittedError("{} metric not found; setting to euclidean".format(self.X2_metric))
            self.X2_metric = 'euclidean'
            simfunc = euclid_func
        P = squareform(pdist(np.nan_to_num(self.X2), metric=simfunc))
        return P

    def fit_transform(self, X, X2, **kwargs):
        ...
        X2 : array, shape=[n_samples, n_features2]
            input data with `n_samples` samples and `n_dimensions2`
            dimensions. Accepted data types: `numpy.ndarray`,
            `scipy.sparse.spmatrix`, `pd.DataFrame`
            These are the exogenous features matrix
        
        kwargs : further arguments for `EPHATE.transform()`
            Keyword arguments as specified in :func:`~ephate.EPHATE.transform`
        Returns
        -------
        embedding : array, shape=[n_samples, n_dimensions]
            The cells embedded in a lower dimensional space using EPHATE
        """
        with _logger.task("fitting EPHATE"):
            self.fit(X, X2)
    ...
    def fit(self, X, X2):
        ...
        print(X.shape, X2.shape, n_pca)

        if precomputed is None:
            _logger.info(
                "Running EPHATE on {} samples; {} features for view 1 and {} features for view 2.".format(
                    X.shape[0], X.shape[1], X2.shape[1]
                )
            )
        else:
            _logger.info(
        ...
        if self.graph is None:
            with _logger.task("PHATE graph and diffusion operator"):
                self.graph = Graph(
                    X,
                    n_pca=n_pca,
    ...
    def transform(self, X=None, t_max=100, plot_optimal_t=False, ax=None):
        ...
        if self.graph is None:
            ...
            warnings.warn(
                "Pre-fit EPHATE should not be used to transform a "
                "new data matrix. Please fit EPHATE to the new"
                " data by running 'fit' with the new data.",
                RuntimeWarning,
            )
            if (
                isinstance(self.graph, graphs.TraditionalGraph)
    ...
    def _parse_input(self, X, X2):
        ...
        if isinstance(X, graphs.LandmarkGraph) or (
            ...
            if isinstance(self.graph, graphs.TraditionalGraph):
                ...
                precomputed = None
            return X, X2, n_pca, precomputed, update_graph
        ...
        # make sure the X2 matrix has normalized features
        if not utils.is_Symmetric(X2) and self.normalize_exog_features:
    ...
    def _check_params(self):
        """
        Check PHATE and EPHATE parameters 
        This allows us to fail early before minutes of runtime.
        Raises
        ------
        ...
        utils.check_if_not(None, utils.check_positive, utils.check_int, n_landmark=self.n_landmark, 
        n_pca=self.n_pca, knn_max=self.knn_max
        )
        utils.check_if_not("auto", utils.check_positive, utils.check_int, t=self.t)
        acceptable_distance_metrics = ['braycurtis', 'canberra', 'chebyshev', 'cityblock', 'correlation', 'cosine', 'dice', 'euclidean', 
        'hamming', 'jaccard', 'kulsinski', 'l1', 'l2', 'mahalanobis', 'manhattan', 'matching', 'minkowski', 'precomputed', 
        'precomputed_affinity', 'precomputed_distance', 'rogerstanimoto', 'russellrao', 'seuclidean', 'sokalmichener', 'sokalsneath', 'sqeuclidean', 'yule']
        if not callable(self.knn_dist):
            utils.check_in(acceptable_distance_metrics, knn_dist=self.knn_dist)

        if not callable(self.mds_dist):
            utils.check_in(acceptable_distance_metrics, mds_dist=self.mds_dist)

        utils.check_in(['euclidean','cosine','pearsonr','spearmanr'], X2_metric=self.X2_metric)
