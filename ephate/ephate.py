...
class EPHATE(BaseEstimator):
    ...
    def compute_pairwise_similarity_mtx(self):
        ...
        if self.X2_metric == 'cosine':
            simfunc = lambda u,v : np.dot(u, v)/numpy.linalg.norm(u)*numpy.linalg.norm(v)
        elif self.X2_metric == 'pearsonr':
            simfunc = lambda u,v : np.corrcoef(u,v)[0,1]
        elif self.X2_metric == 'spearmanr':
            simfunc = lambda u,v : spearmanr(u, v).correlation
        elif self.X2_metric == 'euclidean':
            simfunc = lambda u,v : (1 + np.sqrt(np.sum((e1 - e2)**2 for e1,e2 in zip(u, v)))) ** -1
        else:
            raise NotFittedError("{} metric not found; setting to euclidean".format(self.X2_metric))
        self.X2_metric = 'euclidean'
        simfunc = lambda u,v : (1 + np.sqrt(np.sum((e1 - e2)**2 for e1,e2 in zip(u, v)))) ** -1
        # puts into the pairwise distance function, using the given fnuction
        P = squareform(pdist(np.nan_to_num(self.X2, simfunc)))
